...
app.put("/edit_photo_in_gallery", async (req: any, res) => {
  ...
  try {
    ...
    console.error("Wystapil blad", error);
    return res.status(500).json({ error: "Wystapil blad podczas edycji zdjęcia w galerii" });
  }
});
...
app.put("/updateCarMedia", async (req, res) => {
  ...
  return res.send(await Car.findOne({ _id: carId }).populate('Car_Specification').populate('media'));
...
app.listen(3000, () => {
  console.log('Serwer nasłuchuje na porciea 3000');
});
