...
const AddPhotoToProfileGallery = () => {
    ...
    const deleteOldProfileImage = async () => {
      ...
      if(oldProfileImgId)
        await axios.delete("http://145.239.93.11:3000/delete_car_media", { data: { _id: oldProfileImgId, _car_id: car?._id }})*/
      await axios.delete("http://145.239.93.11:3000/delete_car_media", { data: { _car_id: car?._id }})
    }
    ...
    const sendPhotoToDatabase = async (name: string, fullUrl: string) => {
      ...
      await axios.post("http://145.239.93.11:3000/post_photo_to_gallery", postData, {
        headers: {
