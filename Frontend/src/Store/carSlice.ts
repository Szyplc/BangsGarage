...
export const getLikedCars = createAsyncThunk<CarData[]>(
  "car/getLikedCars",
  async () => {
    const arrayLikedCars: string[] = (await axios.get("http://145.239.93.11:3000/get_liked_cars")).data
...
const getCarToShowFunction = async (index: number): Promise<{ car: CarData, isLike: boolean } | null> => {
  const newCar = await (await axios.get("http://145.239.93.11:3000/getCarToSlider", { params: { index: index }})).data as CarData | null
  if(newCar) {
    ...
    let isLike = (await axios.get("http://145.239.93.11:3000/check_if_user_like_car", 
...
export const getCarsId = createAsyncThunk<string[]>(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get<string[]>("http://145.239.93.11:3000/getUserCars");
...
export const getCarData = createAsyncThunk<CarData | null, string>(
  ...
  async (car_id, { rejectWithValue }) => {
    try {
      const response = await axios.get<CarData>(`http://145.239.93.11:3000/getCarData`, {
        params: { car_id },
      });
      return response.data;
    } catch (error) {
